...
use Modules\Utils\Utilitaries\Crypto;
use Modules\Utils\Exceptions\ApiException;
use Modules\Utils\Exceptions\ValidatorException;
...
class User extends Authenticatable implements MustVerifyEmail, Transformable, AuditableContract
{
    ...
    protected static $basicPermissionGroup = 'Cliente';
    ...
    public static function create($data)
    {
        ...
        try {
            if (isset($data['avatar'])) {
                $avatar = $data['avatar'];
                unset($data['avatar']);
            } else $avatar = static::$defaultImage;
            ...
            $group = app(PermissionRepository::class)->where('name', self::$basicPermissionGroup)->first();
            if (is_null($group))
                throw new PermissionGroupNotFound(static::$basicPermissionGroup);

            app(PermissionMapService::class)->associate($group, $user);
    ...
    {
        return $this->morphOne(Image::class, 'responsable');
    }
